...
function buildForwardHeaders(req: NextRequest): Headers {
  ...
  // Tell the upstream which origin/host this request originated from so it
  // can build correct redirect URLs if it needs to.
...
export async function proxyAuthToBackend(
  ...
  try {
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      // Build the response manually so Set-Cookie headers come through intact.
      forwardHeaders.set("location", location);
